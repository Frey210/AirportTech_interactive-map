...
export default function MapEditorPanel({ data, markers, selectedId, dirty, saving, error, onSelect, onAdd, onUpdate, onDelete, onSave, onCancel, onReload }: Props) {
  ...
  return <div className="editor-panel">
    ...
    {selected && <section className="marker-inspector">
      <span className="section-label">INSPEKTOR PENANDA</span><h2>{selected.peralatan.nama_peralatan}</h2>
      <fieldset className="icon-palette"><legend>Ikon</legend>{data.ikon.map((icon) => <button type="button" key={icon.id} onClick={() => onUpdate(selected.id, { ikon: { id: icon.id, nama: icon.nama, file_url: icon.file_url } })} aria-pressed={selected.ikon.id === icon.id}><img src={icon.file_url} alt="" width="28" height="28" /><span>{icon.nama}</span></button>)}</fieldset>
      <div className="inspector-grid"><label>Posisi X (%)<input type="number" min="0" max="100" step="0.1" value={Math.round(selected.x_ratio * 1000) / 10} onChange={(event) => onUpdate(selected.id, { x_ratio: clamp(Number(event.target.value) / 100, 0, 1) })} /></label><label>Posisi Y (%)<input type="number" min="0" max="100" step="0.1" value={Math.round(selected.y_ratio * 1000) / 10} onChange={(event) => onUpdate(selected.id, { y_ratio: clamp(Number(event.target.value) / 100, 0, 1) })} /></label></div>
      <div className="range-controls"><label><span>Ukuran</span><output>{Math.round(selected.size_ratio * 1000) / 10}%</output><input aria-label="Ukuran penanda" type="range" min="0.5" max="20" step="0.5" value={selected.size_ratio * 100} onChange={(event) => onUpdate(selected.id, { size_ratio: Number(event.target.value) / 100 })} /></label><label><span>Rotasi</span><output>{Math.round(selected.rotation_deg)}°</output><input aria-label="Rotasi penanda" type="range" min="0" max="359" step="1" value={Math.round(selected.rotation_deg) % 360} onChange={(event) => onUpdate(selected.id, { rotation_deg: Number(event.target.value) })} /></label></div>
      <div className="nudge-controls" aria-label="Geser penanda dengan tombol"><button type="button" onClick={() => onUpdate(selected.id, { y_ratio: clamp(selected.y_ratio - .005, 0, 1) })}>↑ Atas</button><button type="button" onClick={() => onUpdate(selected.id, { x_ratio: clamp(selected.x_ratio - .005, 0, 1) })}>← Kiri</button><button type="button" onClick={() => onUpdate(selected.id, { x_ratio: clamp(selected.x_ratio + .005, 0, 1) })}>Kanan →</button><button type="button" onClick={() => onUpdate(selected.id, { y_ratio: clamp(selected.y_ratio + .005, 0, 1) })}>↓ Bawah</button></div>
      <label>Catatan<textarea maxLength={500} value={selected.catatan ?? ''} onChange={(event) => onUpdate(selected.id, { catatan: event.target.value })} /></label><button className="danger-button" type="button" onClick={() => onDelete(selected.id)}>Hapus penanda</button>
    </section>}
